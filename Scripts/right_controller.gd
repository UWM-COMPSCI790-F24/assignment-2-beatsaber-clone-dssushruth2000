...
@export var sword_color = Color.RED  # Set the sword color for this script
...
func _process(delta: float) -> void:
    ...
    if raycast.enabled and is_instance_valid(raycast) and raycast.is_colliding():
        ...
        if collider and collider.get_parent() != null:  
            #print("Right sword hit: ", collider.name)
            $"RightLaserSword".points[1] = raycast.get_collision_point()  # Update sword endpoint to collision point

            # Check if the collider's parent node has the "split_cube" function
            if collider.get_parent().has_method("split_cube"):
                collider.get_parent().call("split_cube", sword_color)  
